...
public class UserServiceTest {
	...
	public void b_insertTest() {
		...
		UsersVO userVO = UsersVO.builder().userId("Korea").password("12345").userName("대한민국").userRole("gov").build();
		
		int ret = userService.insert(userVO);
		
		// ret값이 1이며 오차가 없는 결과를 받았느냐라고 Text를 수행
		assertEquals(ret, 1, 0); 
		
		userVO = UsersVO.builder().userId("admin").password("12345").userName("홍길동").userRole("admin").build();
...
userVO = UsersVO.builder().userId("guest").password("12345").userName("성춘향").userRole("admin").build();
		
		ret = userService.insert(userVO);
		assertEquals(ret, 1,0 );
		
		
userVO = UsersVO.builder().userId("dba").password("12345").userName("이몽룡").userRole("admin").build();
