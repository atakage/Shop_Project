...
public class UserService {
	...
	public UserService(PasswordEncoder passwordEncoder, 
					UserDao userDao,AuthoritiesDao authDao, MailSendService mailService) {
		super();
	 ...
	 * 
	 * @update 2020-04-10 Map 구조의 VO 데이터를 UserVO로 변경
	 * 
	 */
	@Transactional
	...
	// email 인증방식으로 회원가입을 처리할 것이므로 userVO를 파라미터로 받아서
	// enabled를 false로 처리하고 role 정보는 update 하지 않는 것으로 처리
	// 이후 email 인증이 오면 enabled와 role 정보를 설정하도록 함
	/*
	 * @since 2020-04-20
	 * @author userVO
	 * @param userVO
...
//		boolean bRet = mailService.join_send(userVO);
		...
		String sRet;
		try {
			sRet = mailService.join_send(userVO);		// 메일 보내짐
	 ...
	 * @since 2020-04-21
	 * 회원 정보를 받아서 DB에 저장하고
	 * 회원 정보를 활성화할 수 있도록 하기 위해 인증 정보를 생성한 후 Controller로 Return
	 * @param userVO
	 * @return
	 */
	public String insert_getToken(UserDetailsVO userVO) {
		...
		*/
		...
		log.debug("암호화 전:"+ email_token);
		String enc_email_token = PbeEncryptor.getEncrypt(email_token);
		log.debug("암호화 후:"+ enc_email_token);
