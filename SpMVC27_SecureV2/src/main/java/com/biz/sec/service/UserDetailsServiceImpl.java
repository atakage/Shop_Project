...
public class UserDetailsServiceImpl implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		userDetails.setCredentialsNoneExpired(true);
		
		userDetails.setAuthorities(this.getAuthorities(username));
		
		// builder pattern이용
		userDetails = UserDetailsVO.builder().username(userVO.getUsername()).password(userVO.getPassword()).enabled(true).accountNonExpired(true).accountNonLocked(true).credentialsNoneExpired(true).authorities(this.getAuthorities(username)).build();
