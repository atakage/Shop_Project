...
public interface ProOptionsDao {
	...
	/*
	 * mysql auto_increment로 되어 있는 s_seq 컬럼을 @Options로 설정을 해주면 insert를 수행한 후 ProSizeVO의 s_seq 변수에 새로 생성된 s_seq값을 담아서 이후
	 * Service, Controller 등에서 사용할 수 있도록 만들어 줌
	 */
	...
	@Select("SELECT * FROM tbl_pro_color LEFT JOIN tbl_options ON c_color = o_standard WHERE size_seq = #{longSeq} ORDER BY o_name")
