...
public class ProductController {
	 ...
	 * jsp form을 최초로 열어서 보여주는 method에서 modelAttribute에 해당하는 vo객체를 생성하고
	 * model에 담아서 jsp로 보내야 함
	 * 
	 * controller 클래스에
	 * @ModelAttribute() method를 작성해 두면
	 * 나머지 method에서는 별도로 model에 vo를 담아서 보내지 않아도 새로 생성된 vo를 만들어서
	 * 이 method가 jsp로 보내주는 역할을 대신함
	 */
	...
	// product/, product/list로 요청했을 때 처리할 method
	...
	// overloading: method 중복 정의, 같은 이름 method 매개변수 다르게
	@RequestMapping(value="/insert", method=RequestMethod.GET)
	public String insert(ProductVO productVO ,Model model) {
		return "product/pro_write2";
	...
	@ResponseBody
	@RequestMapping(value="/color_insert", method=RequestMethod.POST)
	public Object insert_size(ProColorVO proColorVO) {
		
		log.debug("COLOR: " + proColorVO.getC_color());
		log.debug("SIZE_SEQ: " + proColorVO.getSize_seq());
	...
	public Object get_color_list_by_size(@RequestParam("s_seq") String s_seq) {
