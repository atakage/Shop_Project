...
public class BBsController {
	 ...
	 * 	BBsService 인터페이스를 거쳐서 연결이 되어 있음
	 * 	BBsService 인터페이스를 imp한 클래스의 코드가 미완성 상태이지만
	 * 	BBsController 입장에서는 BBsService *의 코드가 완성 되어 있다라는 전제하에 Controller 코드 작성을 진행할 수 있음
	 * 	만약 이후에 BBsService* 의 코드가 완성되고 여러개의 BBsServiceImp* 클래스가 작성될 시 필요한 클래스를 가져다 부착하면 됨
	 * 
	 *  결합도를 낮추는 결과가 됨
	 *  결합도는 낮게 응집도는 높은 모듈 연계가 좋은 설계 
	 *  * 결합도: 모듈간 의존도, 모듈 코드 작성의 유연성과 관련
	 *  
	 */
	 ...
	 * BBS_LIST라는 이름으로 model에 Attribute를 추가하여 bbs_list.jsp와 rendering하도록 수행함
	 * 
	 * 
	 */
	...
	/*
	 * 리스트에서 
	 * 글쓰기(작성) 버튼을 클릭했을 때 게시판 작성 화면을 열어줄 path
	 * 버튼을 클릭했을 때 작동할 path는 method를 GET로 설정함
	 */
	 ...
	 * - 저장 버튼을 클릭하면 <form> tag에 지정된 action path method에 지정된 방식으로 데이터를 전송함
	 * - 이러한 버튼의 기본 기능을 submit이라고 함
	 *  bbs_write.jsp에서는 <form> tag에 action을 제거
	 * 	- 이렇게 되면 처음 게시판 작성화면을 열기 위해 사용했던 path인 ${rootPath}/bbs/insert가 action에 자동으로 설정이 됨
	 * 	- 또한 method는 POST로 지정이 되어있기 때문에 암시적으로 ${rootPath}/bbs/insert path로 POST 방식으로 데이터를 전송하라는 설정과 같음
	 *  그래서 이 메서드에서 요청을 수신하게 되고 input tag에 입력된 문자열들은 bbsVO 객체변수에 담기게 됨
	 * 
	 */
	...
	// 게시판 리스트에서 제목을 클릭하면 update path에[게 b_id값을 전달하고 b_id값을 수신하여 게시판에서 1개의 게시판 데이터를 SELECT하고 bbsVO에 받음
	// model에 bbsVO를 bbs라는 이름으로 attribute를 추가하고 bbs_write.jsp에 전달
	...
	public String fileUp(MultipartFile upFile) {
		...
		log.debug("업파일: "+upFile.getOriginalFilename());
