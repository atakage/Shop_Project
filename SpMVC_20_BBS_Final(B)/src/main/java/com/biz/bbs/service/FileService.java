 ...
 * 일단 서버에 파일을 업로드 하고 파일 이름을 다시 내려보내서 base64로 encoding된 파일 정보를 img src="저장된경로/파일이름" 형식으로 변경
 * 
 */
...
public class FileService {
	...
	private final String filePath; 	// context 불러오는 부분?
	
	
	
	/*
	 * 브라우저에서 파일이 전송되어 오면
	 * 원래 파일 이름을 UUID가 부착된 파일이름으로 변경예정
	...
	public String fileUp(MultipartFile upFile) {
		...
		// upload할 filePath가 있는지 확인을 하고 없으면 폴더를 생성
